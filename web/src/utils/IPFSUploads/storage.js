 ...
 * @property {string} fileURI an ipfs:// URI for the image file
 * @property {string} metadataURI an ipfs:// URI for the metadata file
 * @property {string} fileGatewayURL an HTTP gateway URL for the image
 ...
 */
export async function storeFile(imageFile, caption) {
  ...
  const cid = await client.put([imageFile[0]], {
  ...
  const fileGatewayURL = makeGatewayURL(cid, imageFile[0].name);
  const fileURI = `ipfs://${cid}/${imageFile[0].name}`;
  const metadataURI = `ipfs://${cid}/metadata.json`;
  return { cid, metadataGatewayURL, fileGatewayURL, fileURI, metadataURI };
}
